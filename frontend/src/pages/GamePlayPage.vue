...
const rhythmMine = ref({ score: 1240, combo: 12, hearts: 5 })
const rhythmOpponent = ref({ score: 1180, combo: 8, hearts: 5 })
...
function toResult(isFailed = false) {
  if (!game.value) return
  const resultQuery = { ...route.query }
  delete resultQuery.result
  const resultState =
    isFailed && isRhythmDuel.value ? 'lose' : isFailed ? 'failed' : undefined
  ...
  void submitPlayedResult({
    ...
    outcome: isFailed ? 'LOSE' : undefined,
  })
  router.push({
    name: 'game-result',
    params: { gameId: game.value.id },
    query: resultState ? { ...resultQuery, result: resultState } : resultQuery,
  })
}

function toHoldRecordMissedResult() {
  if (game.value?.id !== 'hold') return

  const resultQuery = { ...route.query, result: 'not-new-record' }
  void submitPlayedResult({
    gameSlug: game.value.id,
    mode: mode.value,
    startedAt: playStartedAt,
    score: holdElapsedTenths.value,
    outcome: 'LOSE',
  })
  router.push({
    name: 'game-result',
...
function handleRhythmInput(
  ...
  if (isMiss) {
    ...
    if (playerStatus.hearts === 0) toResult(player === 'mine')
    ...
    >
      ...
      <aside v-else-if="game.id === 'hold'" class="info-panel eye-see-info">
        <header class="eye-see-camera-header">
          <p>나의 웹캠</p>
          <span :class="{ 'eye-see-camera-header__status--ready': isLocalCameraActive }">
            {{ isLocalCameraActive ? '연결됨' : '준비 중' }}
          </span>
        </header>
        <div class="eye-see-camera">
          ...
          <div class="eye-see-camera__timer" aria-live="polite">
            <span>현재 생존 시간</span>
            <strong>{{ holdElapsedLabel }}</strong>
          </div>
        </div>
      ...
      <section class="gameplay-board" aria-live="polite">
        ...
        <template v-else-if="game.id === 'blink'">
          <section class="blink-stage" aria-label="눈 깜빡이기 플레이 영역">
            ...
            <section class="blink-stage__stat-card blink-stage__time-card">
              <span>남은 시간</span>
              <strong>{{ session.timeLabel }}</strong>
        ...
        <template v-else-if="game.id === 'hold'">
          ...
          <div class="hold-controls">
            ...
            ><button type="button" @click="eyeSeeState = 'success'">
              성공 (mock)</button
            ><button type="button" @click="toHoldRecordMissedResult">
              실패 (mock)
            </button>
          </div>
        </template>
      ...
      >
        ...
        <p
          class="camera-state"
          :class="{ 'camera-state--ready': isLocalCameraActive }"
        >
          {{ isLocalCameraActive ? '카메라 연결됨' : '카메라 준비 중' }}
        </p>
        <p v-if="game.id === 'draw'" class="webcam-panel__hint">
          얼굴과 눈이 화면 안에 함께 보이도록<br />카메라 위치를 조정해 주세요.
        </p>
        <p v-if="game.id === 'rhythm'" class="webcam-panel__rhythm-tip">
      ...
      <aside v-else-if="game.id === 'air'" class="air-players-panel">
        ...
        <article class="air-player-card you">
          ...
          <p
            class="air-player-card__camera-status"
            :class="{
              'air-player-card__camera-status--ready': isLocalCameraActive,
            }"
          >
            {{ isLocalCameraActive ? '카메라 연결됨' : '카메라 연결 대기' }}
          </p>
        </article>
      </aside>
      <aside
      ...
      <aside v-else-if="isCompetitive" class="info-panel opponent-panel">
        <template v-if="game.id === 'hold'">
          ...
          <p
            class="camera-state"
            :class="{ 'camera-state--ready': hasPeerCamera }"
          >
            {{
              hasPeerCamera
                ? '친구 카메라가 연결되었습니다.'
...
.gameplay-layout--air-ai .hockey-rink {
...
@media (max-width: 680px) {
  ...
  .gameplay-layout--blink-solo .blink-stage {
    height: clamp(420px, calc(100svh - 160px), 480px);
    min-height: 0;
  }
  .blink-stage__stat-card,
  .blink-stage__tip {
  ...
  .blink-stage__count-card {
    top: 14px;
    width: min(180px, calc((100% - 48px) / 2));
  }
  .blink-stage__time-card {
    left: 14px;
  }
  ...
  .blink-stage__count-card .blink-stage__tip {
    padding: 8px 0 0;
    border-radius: 0;
  }
  .blink-stage__stat-card > strong {
