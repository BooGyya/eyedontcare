...
function handleSelectMode(mode: GamePlayMode) {
  ...
  if (mode.id === 'friends' || mode.id === 'random') {
    if (!auth.isAuthenticated) {
      auth.openLogin()
      showToast('ÏπúÍµ¨ ?ÄÍ≤∞Í≥º ?úÎç§ Îß§Ïπ≠?Ä Î°úÍ∑∏?????¥Ïö©?????àÏñ¥??')
      return
    }
  ...
  // AI ?úÏù¥?ÑÍ? ?àÎäî Í≤åÏûÑ(ÏßÄÍ∏àÏ? ?àÏã∏?ÄÎø??Ä Î∞îÎ°ú ?¥Îèô?òÏ? ?äÍ≥† ?úÏù¥?ÑÎ???Í≥†Î•¥Í≤??úÎã§.
...
function handleEnterRoom(payload: {
  ...
  router.push({
    ...
    query: {
      mode: payload.mode,
      // Ï¥àÎ?Î∞©Ï? 4?êÎ¶¨ Î∞?ÏΩîÎìú, ?úÎç§Î∞©Ï? Îß§Ïπ≠?ºÎ°ú Î∞õÏ? ?§Ï†ú roomId(UUID)Î•??òÍ∏¥??
...
<template>
  <section v-if="game" class="game-detail-page">
    ...
    <section class="game-detail-page__hero">
      <div class="game-detail-page__art">
        ...
        >
        <img :src="game.image" :alt="`${displayTitle} Í≤åÏûÑ ?ºÎü¨?§Ìä∏`" />
      </div>
      ...
      <div class="game-detail-page__intro">
        ...
        <dl class="game-detail-page__stats">
          ...
          <div class="game-detail-page__stat">
            ...
            <div>
              <dt>{{ game.durationLabel ?? '?úÌïú ?úÍ∞Ñ' }}</dt>
              <dd>{{ game.duration }}</dd>
            </div>
          </div>
        </dl>

        <div class="game-detail-page__tags" aria-label="Í≤åÏûÑ ?úÍ∑∏">
          <span
    ...
    <section class="game-detail-page__other-games" aria-label="?§Î•∏ Í≤åÏûÑ Î™©Î°ù">
      <h2>?§Î•∏ Í≤åÏûÑ???àÏñ¥??/h2>
      <div class="game-detail-page__other-games-grid">
        ...
        >
          <span class="game-detail-page__other-game-thumb">
            <img
              :src="other.image"
              :alt="`${toDisplayTitle(other.title)} ?∏ÎÑ§??"
            />
          </span>
      ...
      >
        ...
        >
          <div class="game-detail-page__steps">
            <h2 id="game-description-title">Í≤åÏûÑ Î∞©Î≤ï</h2>
            <ol>
              <li v-for="(step, index) in game.steps" :key="step">
                <b>{{ index + 1 }}</b>
                <span>{{ step }}</span>
    ...
    <Transition name="dialog-pop">
      ...
      >
        ...
        >
          <h2 id="ai-difficulty-title">?úÏù¥?ÑÎ? ?†ÌÉù?òÏÑ∏??/h2>
          <p class="game-detail-page__difficulty-subtitle">
            AIÎ≥¥Îã§ ?§Îûò Î≤ÑÌã∞Î©??πÎ¶¨?¥Ïöî. Î™©Ìëú ?úÍ∞Ñ??Í≥®ÎùºÏ£ºÏÑ∏??
          </p>
  ...
  <section v-else class="game-detail-page__missing">
    <img
      ...
      alt=""
    />
    <h1>Í≤åÏûÑ??Ï∞æÏùÑ ???ÜÏñ¥??</h1>
    <p>Í≤åÏûÑ Î™©Î°ù?êÏÑú ?§Ïãú ?†ÌÉù?¥Ï£º?∏Ïöî.</p>
    <RouterLink class="game-detail-page__missing-cta" to="/games"
      >Í≤åÏûÑ Î™©Î°ù?ºÎ°ú Í∞ÄÍ∏?/RouterLink
    >
  </section>
</template>
